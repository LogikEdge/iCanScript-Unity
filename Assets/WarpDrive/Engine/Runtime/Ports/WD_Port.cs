...
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        switch(Edge) {
            case EdgeEnum.Top:
                ...
                if(LocalPosition.x > parentPosition.width) {
                    LocalPosition.x= parentPosition.width-WD_EditorConfig.PortSize;
                    Parent.IsEditorDirty= true;
                }
                break;
            ...
            case EdgeEnum.Left:
                ...
                if(LocalPosition.y > parentPosition.height) {
                    LocalPosition.y= parentPosition.height-WD_EditorConfig.PortSize;
                    Parent.IsEditorDirty= true;
                }
                break;
            case EdgeEnum.Right:
    ...
    public bool IsNearParent() {
        return GetDistanceFromParent() <= WD_EditorConfig.PortSize*2;
    }

	// ----------------------------------------------------------------------
    public WD_Port GetOverlappingPort() {
        WD_Port foundPort= null;
        Top.ForEachRecursive<WD_Port>(
            (port)=> {
                if(port != this) {
