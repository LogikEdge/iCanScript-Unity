        ...
        for(int i= 0; i < functions.Count; ++i) {
            if(FilterIn(functions[i], upperSearchStr)) {
                ...
                if(memberInfo.IsField) {
                    var niceName= NameUtility.ToDisplayName(memberInfo.ToFieldInfo.DisplayName);
                    ...
                    var niceName= NameUtility.ToDisplayName(memberInfo.ToPropertyInfo.DisplayName);
                    var name= displayNameHeader+niceName+displayNameTrailer;
                    toAdd= new Node(NodeTypeEnum.Property, name, memberInfo);
                } else if(memberInfo.IsConstructor) {
                    var niceName= NameUtility.ToDisplayName(memberInfo.DisplayName);
                    var name= displayNameHeader+niceName+displayNameTrailer;
    ...
    Prelude.Tree<Node> GetParentTree(iCS_MemberInfo desc, Prelude.Tree<Node> tree, ref int treeSize) {
		...
		if(desc.ParentTypeInfo.HideFromLibrary == false) {
	        ...
	        if(!iCS_Strings.IsEmpty(className)) {
                className= NameUtility.ToDisplayName(className);
