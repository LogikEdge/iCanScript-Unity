...
public partial class iCS_IStorage {
    ...
    public bool IsInside(iCS_EditorObject node, Vector2 point) {
        return node.LayoutRect.Contains(point);
    }
    ...
    public float GetDistanceFromNode(iCS_EditorObject node, Vector2 point) {
        ...
        float distance= Vector2.Distance(point, Math3D.TopLeftCorner(node.LayoutRect));
        distance= Mathf.Min(distance, Vector2.Distance(point, Math3D.TopRightCorner(node.LayoutRect)));
        distance= Mathf.Min(distance, Vector2.Distance(point, Math3D.BottomLeftCorner(node.LayoutRect)));
        distance= Mathf.Min(distance, Vector2.Distance(point, Math3D.BottomRightCorner(node.LayoutRect)));
        return distance;
    }
